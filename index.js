...
//         const FindData = (req.body.id) ? { _id: objectId } : {};
//         const data = await db.find(FindData).toArray();
//         console.log(data);
//         resp.send(data);
//     } catch (error) {
...
//         const db = await dbConnection();
//         const objectId = new ObjectId(req.body.id);
//         const result = await db.updateOne({ _id: objectId }, { $set: req.body });
//         // console.log(result);
...
//         }
//         resp.status(200).json({ message: 'records was successfully updatesd' });
//     } catch (error) {
